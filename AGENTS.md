...
## Project-Specific Toolkit Guidance

These conventions apply to the toolkit content in this repository in addition to the broader OMX workflow rules above.

### Live MVP contract
- Keep the canonical first-time path limited to `README.md`, `docs/quickstart.md`, and the scripts under `scripts/`.
- Treat `platform/terraform/`, `platform/kubernetes/`, `platform/workloads/demo-app/`, `.github/workflows/`, and `docs/quickstart.md` as the live MVP path.
- Keep example-only content under `examples/` and `docs/examples/`.
- Keep roadmap and future-depth material under `docs/roadmap.md` and related docs.

### Safe extension boundaries
- Terraform should remain a lightweight infrastructure foundation layer for the local demo path.
- Kubernetes manifests should own demo workload deployment concerns.
- Do not move provider-specific, observability-heavy, GitOps-heavy, or enterprise-depth features into the live path unless separately planned.
- If a change makes the first demo materially harder to complete within about one hour, downgrade it to example-only, docs-only, or post-v1.
